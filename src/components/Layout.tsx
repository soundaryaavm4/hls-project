...
export default function Layout({ children }: { children: React.ReactNode }) {
  const { isOnline, toggleMode, userName, logout, uploadLogs, fileName } =
    useApp();
        ...
        <nav className="flex-1 p-3 space-y-1">
          {navItems.map((item) => (
            ...
            >
              ...
              {item.label}
              {item.to === "/alerts" && (
                <span className="ml-auto text-[10px] bg-critical/20 text-critical px-1.5 py-0.5 rounded-full font-medium">
                  !
                </span>
              )}
            </NavLink>
          ...
          >
            ...
            <div
              className={`w-8 h-4 rounded-full relative transition-colors ${
                isOnline ? "bg-success/30" : "bg-muted"
              }`}
            >
              <motion.div
